...
This FHIR Implementation Guide specifies the technical components of the Generic Function Addressing (GF-Addressing), a national initiative led by the Dutch Ministry of Health, Welfare and Sport (VWS).
GF-Addressing aims to establish a standardized, interoperable system for discovering and sharing current (digital) addresses of healthcare providers, enabling reliable and efficient exchange of health data across healthcare information systems.
...
GF-Addressing follows the IHE [mCSD profile](https://profiles.ihe.net/ITI/mCSD/index.html) ([GF-Adressering, ADR-0](https://github.com/minvws/generiekefuncties-adressering/issues/166)). The [mCSD profile](https://profiles.ihe.net/ITI/mCSD/index.html) provides multiple options for deployment. This guide specifies the choices made for The Netherlands. Most impactful/striking choice are:
...
1. Every care provider registers its addressable entities in its own 'Directory.'
1. Every care provider publishes the endpoint (URL) of its 'Directory' at the national LRZa Directory to make it discoverable.
1. An 'Update Client' uses the LRZa Directory ([GF-Adressering, ADR-7](https://github.com/minvws/generiekefuncties-adressering/issues/155)) and the care provider's Directory to consolidate all data into a local 'Query-Directory'.
...
A Directory is a FHIR server that stores addressable entities. In this guide, two subtypes of Directories are defined:

- An **Administration Directory** is used by a healthcare provider to register and manage its own address
- An **Query Directory** is used by practitioners and systems (EHRs) to search and retrieve addressable entities from multiple healthcare providers.

#### Data Source

The Data Source behaves as a Directory Client and is, responsible for managing of addressable entities within a Directory following the ITI-130 transaction.
Addressable entities MUST conform to the [Data models](#data-models).
A Datasource interacts with a Directory using the [ITI-130 transaction](https://profiles.ihe.net/ITI/mCSD/Transactions/ITI-130.html) to create, update, and delete addressable entities.

In this guide, two subtypes of Data Sources are defined:

- An **Administration Client** is used by a healthcare provider to manage its own addressable entities in its Administration Directory.
- An **LRZa Portal** is used by healthcare providers to register the endpoint (URL) of their Administration Directory at the national LRZa Administration Directory.

The Data Source of the LRZa consists of a user interface for healthcare providers to administer their Administration Directory endpoint (URL).([GF-Adressering, ADR-10](https://github.com/minvws/generiekefuncties-adressering/issues/159) and [GF-Adressering, ADR#167](https://github.com/minvws/generiekefuncties-adressering/issues/167))
...
After consolidation, the Update Client writes the updates to a Query Directory. For each instance, the `meta.source` element is populated with the fully qualified URL of that instance at the originating Administration Directory ([GF-Adressering, ADR-6](https://github.com/minvws/generiekefuncties-adressering/issues/154)). When entities from different Administration Directories reference each other using technical IDs, the Update Client MUST rewrite these references to maintain consistency in the consolidated Query Directory. The Update Client MAY use the same interactions a Administration Client uses to register entities in an Administration Directory.
...
>
> Healthcare records (e.g. conditions, observations, or procedures) will contain links (references) to addressable entities. Some entities may be referenced by an _identifier_ (e.g. a URA or DEZI-number), but most references will use either a local _ID_ (e.g. Patient/880e50a3) or URL (https://somecareprovider.nl/fhirR4/Patient/880e50a3). The (local) IDs and (remote) URLs are definitive and widely supported in the FHIR ecosystem. Identifiers may be harder to resolve, may expose sensitive data (like a Citizen number; Dutch BSN) and can be ambiguous (multiple instances carrying the same identifier).
...
**_This resource type is out-of-scope for this IG-version_**
...
An overview of the _most common_ elements and relations between data models:
