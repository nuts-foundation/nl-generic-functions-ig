...
Construction requirements for these self-assigned identifiers (stability, opacity, unpredictability, uniqueness, and issuer verification) are specified in [Record Identifiers](./record-identifiers.html). The identifier construct of the profiles, an issuer-controlled `system` with an URA `assigner`, corresponds to mechanism (a) of REQ-05 there, combined with the assigner of REQ-06.
